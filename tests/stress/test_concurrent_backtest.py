...
class StressTestRunner:
    ...
    async def simulate_backtest_task(
        self, task_id: str, user_id: str, delay_ms: int
    ) -> Dict:
        """模擬單個回測任務"""
    ...
    async def run_concurrent_test(
        ...
        p50_elapsed = (
            elapsed_times[int(len(elapsed_times) * 0.5)] if elapsed_times else 0
        )
        p95_elapsed = (
            elapsed_times[int(len(elapsed_times) * 0.95)]
            if len(elapsed_times) > 20
            else (elapsed_times[-1] if elapsed_times else 0)
        )
        p99_elapsed = (
            elapsed_times[int(len(elapsed_times) * 0.99)]
        ...
        print(f"\n【監控指標】")
        print(
            f"  最終隊列長度：{self.collector.get_gauge('task_queue_length', {'queue': 'backtest'}) or 0}"
        )
