...
def test_delete_all_completed():
    t = create_task(
        "backtest", {"code": "000003", "_t": uuid.uuid4().hex}, title="done"
    )
    update_task(
        t["task_id"], status=STATUS_COMPLETED, progress=100, result={"ok": True}
    )
...
def test_cleanup_stale_tasks_timeout(monkeypatch):
    ...
    task = create_task(
        "optimize", {"code": "000004", "_t": uuid.uuid4().hex}, title="stale"
    )
    tid = task["task_id"]
    update_task(tid, status=STATUS_RUNNING, progress=50)
    with __import__("src.core.task_manager", fromlist=["_lock"])._lock:
        __import__("src.core.task_manager", fromlist=["_tasks"])._tasks[tid][
            "last_accessed"
        ] = (time.time() - 5)
...
def test_transition_retrying():
    task = create_task(
        "backtest", {"code": "000005", "_t": uuid.uuid4().hex}, title="retry"
    )
    tid = task["task_id"]
...
def test_pipeline_registers_meta(monkeypatch):
    ...
    steps = [
        {
            "task_type": "backtest",
            "params": {"code": "000001", "strategy": "dual_ma"},
            "title": "步驟1",
        },
        {
            "task_type": "backtest",
            "params": {"code": "000002", "strategy": "dual_ma"},
            "title": "步驟2",
            "pass_result": True,
        },
    ]
...
def test_heavy_task_queue_respects_limit(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.core.task_manager._resolve_heavy_max_concurrent", lambda: 1
    )
    gates = []
