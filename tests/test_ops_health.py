...
def test_evaluate_ok_minimal_snapshot():
    ev = evaluate_ops_health({
        ...
        "index_audit": {"ok": True, "missing": [], "present_count": 5, "expected_count": 5},
        "data_sources": {
            "degraded_categories": [],
            "total_categories": 3,
...
def test_build_health_sop_payload_reuses_snapshot(monkeypatch):
    ...
    snap = {"pipeline_metrics": {"x": 1}, "index_audit": {"ok": True, "missing": []}, "data_sources": {}}
...
def test_build_health_sop_payload_shape(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda: {
            "pipeline_metrics": {"cache": {"pending_deferred": 0}},
            "index_audit": {"ok": True, "missing": [], "present_count": 1, "expected_count": 1},
            "data_sources": {
                "degraded_categories": [],
                "total_categories": 1,
                "healthy_categories": 1,
