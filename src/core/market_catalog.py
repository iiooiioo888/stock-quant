...
class MarketInstrument:
    ...
    topbar: bool = False
    asset_class: str = (
        ""  # index | stock | forex | crypto | commodity | etf | rate | structured | money_market | derivative | carbon | alternative
    )
...
def map_price_sources(inst: MarketInstrument) -> tuple[list[PriceSource], str]:
    ...
    if mkt == "exchange":
        ...
        if ex:
            return (
                _ps(ex),
                "交易所產品：以交易所官方口徑為準；個人實時成交價通常在券商端查看。",
            )
        # fallback by group
        if g in ("a_share",):
            return (
                _ps("SSE", "SZSE"),
                "A股：交易所官方披露為準；實時行情以持牌券商端為准。",
            )
        if g in ("hk_stock",):
            return _ps("HKEX"), "港股：交易所披露/行情為準；實時行情以持牌券商端為准。"
        if g in ("commodities",):
            return (
                _ps("SHFE", "DCE", "CZCE", "CFFEX"),
                "期貨/商品：交易所或期貨公司持牌渠道。",
            )
    ...
    if mkt == "interbank":
        return (
            _ps("CHINAMONEY", "CHINABOND"),
            "銀行間市場：交易/利率口徑可參考中國貨幣網；估值口徑以中債為準。",
        )
    ...
    if mkt == "otc":
        ...
        if ac in ("derivative", "structured", "money_market"):
            return (
                _ps("CHINAMONEY", "CHINABOND"),
                "OTC/協議定價：多需機構報價或交易對手報價；此處提供權威市場信息/估值入口。",
            )
    ...
    if mkt == "registry":
        if g in ("alternative",):
            return (
                _ps("CHINAMONEY"),
                "登記/配額類資產：價格視交易機制與合規資格而定；此處提供權威信息入口。",
            )
...
VALID_SCOPES = frozenset(
    ["all", "dashboard", "topbar", "custom", "stocks", "tradeable", *GROUP_ORDER]
)
            ...
            for t in abs_tenors:
                add(
                    _make(
                        ...
                        market=(
                            "otc"
                            if sc in ("ABS_SUPPLYCHAIN", "ABS_LEASING", "CMBS")
                            else "interbank"
                        ),
                        currency="CNY",
                        settlement="OTC",
...
HOME_INDICES = [
    (i.symbol, i.name) for i in MARKET_INSTRUMENTS if i.group in ("asia", "us")
][:8]


def instruments_for_charts(
    scope: str, symbols: str | None = None
