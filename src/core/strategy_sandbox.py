...
BLOCKED_IMPORT_ROOTS: frozenset[str] = frozenset({
    "os", "sys", "subprocess", "shutil", "socket", "http", "https",
    "ftplib", "smtplib", "ctypes", "importlib", "code", "codeop",
    "pickle", "shelve", "dbm", "sqlite3", "multiprocessing", "threading",
    "signal", "mmap", "pathlib", "builtins", "pty", "tty", "fcntl",
    "resource", "tempfile", "webbrowser", "urllib", "requests",
...
FORBIDDEN_CALL_NAMES: frozenset[str] = frozenset({
    "open", "eval", "exec", "compile", "compile_command",
    "__import__", "getattr", "setattr", "delattr",
    "globals", "locals", "vars", "dir", "input", "breakpoint",
    "help", "memoryview", "super",  # super 在策略中不必要，且常被濫用於逃逸
})
...
def _is_allowed_import(module: str | None) -> bool:
    ...
    if root in BLOCKED_IMPORT_ROOTS:
        return False
    if root == "src":
        return module in ALLOWED_SRC_SUBMODULES or module.startswith("src.core.strategy_base")
    return root in ALLOWED_IMPORT_ROOTS


def validate_strategy_source(source: str, *, max_bytes: int = MAX_SOURCE_BYTES) -> StrategyValidationResult:
    ...
    if len(encoded) > max_bytes:
        return StrategyValidationResult(False, f"策略檔案超過大小上限 ({max_bytes} bytes)")
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Import):
            ...
            if attr in ("__import__", "__builtins__", "__globals__", "__code__",
                        "__subclasses__", "__bases__", "__mro__", "__dict__",
                        "gi_frame", "f_globals", "f_locals"):
                return StrategyValidationResult(False, f"禁止訪問屬性: {attr}")

        elif isinstance(node, ast.Subscript):
            if isinstance(node.value, ast.Name) and node.value.id in ("__builtins__", "__globals__"):
                return StrategyValidationResult(False, "禁止訪問 __builtins__ / __globals__")
    ...
    if "UserStrategy" not in source:
        return StrategyValidationResult(False, "策略須繼承 UserStrategy（from src.core.strategy_base import UserStrategy）")

    return StrategyValidationResult(True)


def validate_strategy_file(filepath: str, *, max_bytes: int = MAX_SOURCE_BYTES) -> StrategyValidationResult:
    try:
