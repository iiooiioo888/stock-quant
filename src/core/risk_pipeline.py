...
class PortfolioState:
    ...
    total_capital: float
    cash: float
    positions: dict = field(default_factory=dict)  # {code: {"shares": N, "value": X, "cost": C}}
    nav: float = 0.0
...
class RiskPipeline:
    ...
    def __init__(
        ...
        self.stamp_tax = stamp_tax if stamp_tax is not None else settings.backtest_stamp_tax
        ...
        # 歷史統計（用於 Kelly）
        self._trade_stats: dict[str, dict] = {}  # strategy -> {wins, losses, total_pnl, ...}
    ...
    def process_signals(
        ...
        for code, code_signals in signals_by_code.items():
            ...
            if dd_state["status"] == "停止":
                if signal_type == SignalType.BUY:
                    orders.append(TradeOrder(
                        code=code, side=OrderSide.BUY, shares=0, price=price,
                        strategy=trigger_strategy, signal_strength=composite_strength,
                        position_value=0, risk_status="rejected",
            ...
            if abs(composite_strength) < self.min_signal_strength:
                if signal_type != SignalType.HOLD:
                    orders.append(TradeOrder(
                        ...
                        shares=0, price=price, strategy=trigger_strategy,
                        signal_strength=composite_strength, position_value=0,
            ...
            position_value = self._calculate_position_size(
                code=code, price=price, signal_type=signal_type,
                strength=composite_strength, vol=vol,
            ...
            if signal_type == SignalType.BUY:
                ...
                if shares < 100:
                    orders.append(TradeOrder(
                        ...
                        adjustments={"calculated_value": position_value, "min_lot": 100 * price},
                    ))
                    continue
                # 檢查資金
                cost = shares * price * (1 + settings.backtest_commission)
                if cost > self.portfolio.cash:
                    shares = int(self.portfolio.cash / price / (1 + settings.backtest_commission) / 100) * 100
                    if shares < 100:
                        orders.append(TradeOrder(
                            code=code, side=OrderSide.BUY, shares=0, price=price,
                            strategy=trigger_strategy, signal_strength=composite_strength,
                ...
                order = TradeOrder(
                    code=code, side=OrderSide.BUY, shares=shares, price=price,
                    strategy=trigger_strategy, signal_strength=composite_strength,
                ...
                order = TradeOrder(
                    ...
                    adjustments={"commission": round(commission, 2), "stamp_tax": round(stamp, 2)},
                )
                orders.append(order)
    ...
    def execute_order(self, order: TradeOrder):
        ...
        if order.side == OrderSide.BUY:
            ...
            if order.code in self.portfolio.positions:
                ...
                pos["cost"] = (old_cost + new_cost) / pos["shares"] if pos["shares"] > 0 else 0
            else:
    ...
    def _aggregate_signals(self, signals: list[TradeSignal]) -> Optional[tuple]:
        ...
        sell_strength = sum(abs(s.strength) for s in signals if s.signal == SignalType.SELL)
        ...
        if buy_count > sell_count and buy_strength > 0:
            composite = min(100.0, buy_strength / max(buy_count, 1) * (0.5 + 0.5 * buy_count / len(signals)))
            trigger = max((s for s in signals if s.signal == SignalType.BUY), key=lambda s: s.strength)
            return SignalType.BUY, round(composite, 2), trigger.strategy
        elif sell_count > buy_count and sell_strength > 0:
            composite = min(100.0, sell_strength / max(sell_count, 1) * (0.5 + 0.5 * sell_count / len(signals)))
            trigger = max((s for s in signals if s.signal == SignalType.SELL), key=lambda s: abs(s.strength))
            return SignalType.SELL, round(-composite, 2), trigger.strategy
        else:
            return SignalType.HOLD, 0.0, ""

    def _calculate_position_size(
        self, code: str, price: float, signal_type: SignalType,
        strength: float, vol: float, drawdown_multiplier: float,
    ) -> float:
        """計算倉位金額"""
        ...
        if self.sizing_method == "atr":
            ...
            if atr > 0:
                risk_multiplier = max(0.5, 2.0 - abs(strength) / 100.0)  # 信號越強，止損越緊
            ...
            if vol > 0:
                ...
                base_value = self.sizer.volatility_target(target_vol, vol, current_position)
            else:
                base_value = self.sizer.fixed_fraction(self.fixed_fraction)
    ...
    def _apply_risk_budget(self, code: str, position_value: float, vol: float) -> float:
        ...
        for c, pos in self.portfolio.positions.items():
            ...
            positions.append({"code": c, "value": pos["shares"] * pos.get("cost", 0), "vol": v})
    ...
    def update_trade_stats(self, strategy: str, pnl: float):
        ...
        if strategy not in self._trade_stats:
            self._trade_stats[strategy] = {
                "wins": 0, "losses": 0, "total_win": 0, "total_loss": 0,
            }
        ...
        stats["avg_win"] = stats["total_win"] / stats["wins"] if stats["wins"] > 0 else 1.0
        stats["avg_loss"] = stats["total_loss"] / stats["losses"] if stats["losses"] > 0 else 1.0
    ...
    def get_state(self) -> dict:
        ...
        return {
            ...
            "positions": {k: {kk: round(vv, 2) if isinstance(vv, float) else vv for kk, vv in v.items()} for k, v in self.portfolio.positions.items()},
