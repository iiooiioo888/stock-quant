...
async def auth_update_settings(body: dict, user=Depends(require_auth)):
    ...
    with get_conn() as conn:
        conn.execute(
            "UPDATE users SET settings = ? WHERE id = ?", (settings_json, user.id)
        )
...
async def user_create_watchlist(body: dict, user=Depends(require_auth)):
    ...
    return {
        "success": True,
        "id": cursor.lastrowid,
        "message": f"監控列表 '{name}' 已創建",
    }


@router.put("/api/user/watchlists/{watchlist_id}")
async def user_update_watchlist(
    watchlist_id: int, body: dict, user=Depends(require_auth)
):
    """更新監控列表"""
    import sqlite3
    from src.core.entitlements import check_watchlist_codes_cap
    ...
    with get_conn() as conn:
        ...
        if updates:
            ...
            conn.execute(
                f"UPDATE user_watchlists SET {', '.join(updates)} WHERE id = ?", params
            )
...
async def user_update_alert(alert_id: int, body: dict, user=Depends(require_auth)):
    ...
    with get_conn() as conn:
        ...
        if updates:
            ...
            conn.execute(
                f"UPDATE user_alert_rules SET {', '.join(updates)} WHERE id = ?", params
            )
...
async def admin_change_role(
    target_user_id: int, body: dict, user=Depends(require_admin)
):
    """修改用戶角色（僅管理員）"""
