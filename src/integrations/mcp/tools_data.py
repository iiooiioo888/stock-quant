...
def handle_sq_search_stocks(args: dict) -> str:
    ...
    try:
        ...
        rows, total = query_stock_universe(
            market="a_share", keyword=keyword or None, limit=limit, offset=0
        )
        stocks = [
            {
                "code": r.get("code"),
        ...
        return json_result(
            {"keyword": keyword, "stocks": stocks, "total": total, "shown": len(stocks)}
        )
...
def handle_sq_sector_rotation(args: dict) -> str:
    ...
    try:
        ...
        return json_result(
            {"rotation": _trim_list(rotation, 25), "total": len(rotation), "days": days}
        )
    except Exception as e:
        return error_result(str(e))
...
def handle_sq_backtest_history(args: dict) -> str:
    ...
    try:
        ...
        rows = get_backtest_history(
            code=code, strategy=strategy, limit=min(max(limit, 1), 30)
        )
        slim = []
        for r in rows:
            slim.append(
...
def handle_sq_stock_capital_flow(args: dict) -> str:
    ...
    try:
        ...
        return json_result(
            {"code": code, "flows": _trim_list(flows, days), "total": len(flows)}
        )
    except Exception as e:
        return error_result(str(e))
